...
DECAY_STEPS = options3d_ACPC.params.decay_steps
DECAY_RATE = options3d_ACPC.params.decay_rate
...
def learning_rate_piecewise_decay(init_lr = 1e-4, global_step = None):
    ...
    # values = [1e-3, 5e-4, 2.5e-4, 1.25e-4, 6.25e-5]
    boundaries = [100, 2000, 4000, 8000, 12000, 16000]
    values = [1e-3, 5e-4, 2.5e-4, 1e-4, 5e-5, 1e-5, 5e-6]
    decayed_lr = tf.train.piecewise_constant(global_step, boundaries, values)
    return decayed_lr


def learning_rate_cosine_decay(init_lr=1e-3, global_step=None):
