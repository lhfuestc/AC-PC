...
def theta_like(real_AP, pred_AP):
    """
    :param real_AP:
    :param pred_AP:
    :return: cal theta of the pred_ACPCline and the true_ACPCline
    ...
    True_norm = tf.sqrt(tf.reduce_sum(tf.square(ACPCTrueline), axis=1))
    Pred_norm = tf.sqrt(tf.reduce_sum(tf.square(ACPCPredline), axis=1))
    multisum = tf.abs(tf.reduce_sum(tf.multiply(ACPCTrueline, ACPCPredline), axis=1))
    out = multisum / (True_norm * Pred_norm + 0.0001)
...
def loss_l2_angle(real_AP, pred_AP):
    ...
    :return:
    '''
    with tf.name_scope('l2_loss_angle'):
        lamd = 0.5
        theta = theta_like(real_AP, pred_AP)
        #
        # mse_loss = lamd * tf.losses.mean_squared_error(real_AP, pred_AP) + (1.0 - lamd) * theta
        mse_loss = tf.losses.mean_squared_error(real_AP, pred_AP)
        loss = lamd * mse_loss + (1.0 - lamd) * theta
        tf.add_to_collection(TOTAL_LOSS_COLLECTION, loss)
