...
#test_model_dir = 'train_1/train_logs_2/'
...
def testMain():
    ...
    Img_Batch = tf.placeholder(dtype= tf.float32, shape=[None, img_W, img_H, img_D, nChann])
    print(termcolor.colored("Building Computation Graph...", 'green', attrs=['bold']))

    pred_model = model.Model_3D(Img_Batch, False)
    ...
    while img_index < num_images:
        ...
        Bata = random.randint(-10, 10)
        # print("Z旋转角度为：", Bata, "°")
        Bata = Bata * np.pi / 180
        val_img, val_lab, names = input_3DImage.get_testdata_MRI(data_dir, test_Img_dir, test_Lab_dir,
                                                                   1, theta, Bata)
        #inp_img = val_img[0]
        #inp_lab = val_lab[0]
        ...
        a = int(round(val_lab[0, 0]))
        b = int(round(val_lab[0, 1]))
        c = int(round(val_lab[0, 2]))
        d = int(round(val_lab[0, 3]))
        e = int(round(val_lab[0, 4]))
        f = int(round(val_lab[0, 5]))

        value = [str(img_index), a, a1,b, b1, c, c1,d, d1, e, e1,f,  f1]
        ...
        plt.scatter(b, a, color='', marker='o', edgecolors='g')  #AC真实
#color='', marker='o', edgecolors='g'
        plt.scatter(b1, a1, color='', marker='o', edgecolors='r')  # AC预测
        plt.subplot(2, 2, 2)
        plt.imshow(val_img[0, :, :, f, 0], cmap='gray')
        plt.scatter(e, d, color='', marker='o', edgecolors='g')  #PC真实
        plt.scatter(e1, d1, color='', marker='o', edgecolors='r')  # PC预测
        plt.subplot(2, 2, 3)
        ...
        img_index += 1
    column_name = ['filename', 'T_ACx', 'P_ACx', 'T_ACy' ,'P_ACy', 'T_ACz', 'P_ACz', 'T_PCx','P_PCx', 'T_PCy','P_PCy', 'T_PCz','P_PCz']
