...
def get_data_MRI(data_dir, file_dir, acpc_dir, batch_size):
    ...
    x_data = np.zeros((batch_size, height,  width,  depth), np.float32)
    ...
    for i in range(begin, end):
        ...
        tfAC = np.dot([ACX, ACY, ACZ, 1], tfA1)
        tfPC = np.dot([PCX, PCY, PCZ, 1], tfA1)
        ...
        # print(tfAC[0], tfAC[1], tfAC[2], tfPC[0], tfPC[1], tfPC[2])
        ...
        #FA_data = transform.resize(FA_data, (width, height, depth)) * 10.0
...
# i = 0
# while i < 32:
#     x_, y_, _ = get_data_MRI(valid_dir, valid_data, valid_label, 3)
#     i += 1
...
def getvaliddata(data_dir, file_dir, acpc_dir, batch_size):
    ...
    for i in range(begin, end):
        ...
        #FA_data = transform.resize(FA_data, (width, height, depth))
        # x_data = np.append(x_data, np.reshape(FA_data, (height,  width,  depth, channel)))  # (image.data, dtype='float32')
        # FA_data = FA_data[:, :, :, np.newaxis]
        x_data[index] = FA_data
        y_data[index][0] = tfAC[0]  # assign 1 to corresponding column (one hot encoding)
