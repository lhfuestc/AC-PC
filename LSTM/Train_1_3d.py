...
def train():
    ...
    Img_Batch = tf.placeholder(tf.float32, shape=[None, img_W, img_H, img_D])  # [None, 256*256*150]
    ...
    pred_model = md3.moulti_cnn_lstm(Img_Batch, True)
    ...
    for i in range(EPOCH):
        for k in index:
            ...
            # batch_train_lab = batch_train_lab
            ...
            # 验证集
            if (step == 0) or ((step + 1) % 100 == 0) or ((step + 1) == max_steps):
                val_data, val_label = input_3DImage.getvaliddata(valid_dir, valid_data, valid_label, 2)
                ...
                with open("Records/valid_records.txt", "a") as file:
                    ...
                    file.write(str(format_str) % (step + 1, valid_loss, val_ACError, val_PCError, angelACPCline,
